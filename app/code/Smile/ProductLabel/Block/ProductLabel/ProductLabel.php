...
class ProductLabel extends Template implements IdentityInterface
{
    ...
    public function getLabelLink() 
    {
        ...
        foreach ($images as $image) {
            ...
            if (in_array(Cat::ENERGY_IMAGE_ATTRIBUTE,$types)){
                $filePath = $image->getFile();
                $fileUrl = $this->storeManager->getStore()->getBaseUrl(\Magento\Framework\UrlInterface::URL_TYPE_MEDIA) . 'catalog/product' . $filePath;
                return $fileUrl;    
            }
        }
            return null;
